...
The test asserts both existence AND the published totals: `sum(agency total-bucket facts)`
must equal the sheet's published Total-row value for each measure and FY. This is what
catches a misaligned column offset (a non-empty-but-wrong test would not) and enforces
the "never invent a number" contract. These totals are direct reads from each file's
published Total row (verified empirically across all 7 files):
...
IMPORTANT layout facts (verified empirically across all 7 files, `sum(agency rows)` ==
published Total row for every measure/FY):

- Two-row header: row 0 = measure-group name repeated across its P/O/T/% columns, row 1
  = the P/O/T/% sub-header (text varies: "P"/"O"/"T" in some years, "Personal"/"Other"/
  "Total" in others — read P/O/T **by position**, ignore the text).
- **Take the FIRST column index whose header matches a measure-group name** — that is the
  group's P column; P/O/T are at `i, i+1, i+2`. (A last-match-wins scan would land on the
  trailing % column and misalign every measure — do not do that.)
- Data rows start at `rows[3:]` (row 2 is a repeated banner row, e.g. the portfolio name
  filled across the row — same skip `_agency_facts` uses). Row 0/1 are the two-row header.
- Skip agency names starting `x`/`xx` and the `Total` row, same as `_agency_facts`.
...
In `normalise_all`, after the existing "Request numbers" extraction for each annual file
(the loop over `[("2019-20", ...), ...]`), add:
...
And after the 2025-26 cumulative "Request numbers" extraction, the same two sheets exist;
call `_parse_pot_sheet` with `fy="2025-26"` (note: `year` is not in scope there — the
cumulative file block uses the literal `"2025-26"`).
